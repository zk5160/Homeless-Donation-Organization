...
import { BehaviorSubject, catchError, firstValueFrom, lastValueFrom, map, Observable, of, Subscription, tap, throwError } from 'rxjs';
...
export class CurrentUserService {
 ...
 constructor(private userService: UserService, private needService: NeedService, private location: Location) { }
  ...
  getCurrentUserCartSize(){
    this.userService.searchUsers(CurrentUserService.currentUser).subscribe((users : User[]) => {CurrentUserService.currentUserCart = users[0].basket});
    var sum = 0;
    ...
    async addItemsToCart(product: Need, numitems: number){
      for (let i = 0; i <= CurrentUserService.currentUserCart.length; i++) {
        ...
        this.needService.getNeed(Math.abs(product.id)).subscribe(inventoryVariant=>{this.needService.updateNeed(
          ...
          type: product.type} as Need).subscribe()});
  
          this.userService.updateUser(
            {id : this.getCurrentUserID(), 
            name: this.getCurrentUser(),  
            basket: (this.getCurrentUserCart())} as User).subscribe();
